...
The FH model has been trained on in-sample domains which have both geospatial and household data available. However, by definition, only geospatial data is available in out-of-sample domains. By learning the relationship between the most significant geospatial covariates and the wealth index (as determined in step 4), the model can estimate what the wealth index would be in an area that has not been surveyed in the DHS.
...
Spatial data has a slightly different structure to standard data frames that you may have worked with in the past, say a household survey, in that (as the name gives away) it is indexed to a specific geographical location in space. It still conatins data values, but a lot of the attributes of these spatial objects are used to describe its geographical location, so that we can position our data values adequately on the surface of the Earth (aside: while our analysis will be confined to objects on Earth, GIS analytics can very well be applied to other objects in Space, such as [NASA's Interactive GIS map of Mars](https://astrogeology.usgs.gov/facilities/mrctr/mars-online-map)).
...
Recall that geo-spatial data is simply a set of data values that are indexed to a geographical location on Earth. In that sense, traditional census or surveys are sources of geo-spatial data as long as they contain information on administrative units, GPS location, or any other geographical identifiers. They can even be more precise, for instance if we are interesetd in measuring a building and its properties.
...
These concepts may seem a little abstract for now, so let us consider visually what each of these processes will entail. Figure XX shows the example of processing geo-spatial data on nightlights intensity for Bangladesh - a process we will be implementing in Section 3. Night lights have been known to be a good proxy for economic development and will be one of our predictors of poverty.
...
You download the datasets from the [DHS website](https://dhsprogram.com/Countries/Country-Main.cfm?ctry_id=1&c=Bangladesh&Country=Bangladesh&cn=&r=4). From this link you will need to click on the 2014/17 Bangladesh survey. The datasets we are interested are the household recode since this gives the data on poverty in the country. We are also interested in the Bangladesh shapefile dataset since this gives the co-ordinates of every upazila. To download these files you will need to register with DHS and give a brief explanation of what you intend to do with the data. For this you can explain that you wish to generate spatially disaggregated poverty estimates for the country. Top tip: when requesting access to the data, remember to tick the box requesting geospatial data as well.

Note that when applying to the DHS for data they say that they will get back to you with their decision within 2 business days (for us it was just a matter of a couple of hours) but be mindful that you should apply for the data a couple of days before you decide to follow this guide.

**Insert screenshot of the DHS website here with circles around the files you wish to download**

First of all you have to generate the unique domain identifier for each observation so that you are able to identify each observation by both its upazila as well as its cluster. **Insert Ida's method here:** This method here should be able the data join so you have the geolocation connected to the household data.
...
Loading a file that combines the household recode module for the 2014 DHS, the geo-location of clusters of households, and upazila codes to which the clusters belong. GPS coordinates correspond to the centroid of a cluster of households, which has a random offset of 2km in urban areas and 5-10km in rural areas to preserve the privacy of households. However, the DHS note that efforts have been made for offsets to be specified in a way that preserves the original administrative units within which HHs reside - therefore this should not affect our matching of households to upazilas.

This dataset has information on: Wealth index factor score, quintiles of the wealth index, strata, household weights, upazila, GPS location of clusters, cluster numbers.

First we want to generate our variable of interest. In our case we use the variable 'prop_wealth_1'. This is the probability that a household will be in the lowest quintile of wealth across the whole survey. By definition 20% of households across the country will be in this bottom quantile but you will be able to see which areas are poorer by seeing which areas across the country have a higher probability of a household being in this bottom quintile of wealth. 

Our variable 'binary_wealth' is a dummy variable that equals 1 if the household is a part of the poorest quintile of wealth in Bangladesh and is equal to 0 otherwise.


We also createvariable to determine how many clusters are in each upazila because depending on this, the estimates will be more or less precise due to the change in sample size. The number of clusters in each upazila is necessary so that we know which method we will use to calculate our direct estimates in the next step.
...
# Test - construct a binary variable for whether a HH belongs to the lowest wealth quintile
...
```
Now can generate the direct estimates and the sampling variances. The method is different if there is 1 cluster per upazila since by definition there will be the same variation in the cluster as in the whole upazila. Therefore we use a design effect if there is only one upazila in the cluster, if there is more than 1 cluster in the upazila we are able to calculate using the survey() package.

First of all we calculate the direct estimates and the sampling variances for every upazila using the survey package, regardless of how many clusters are in the area. We will then overwrite the direct estimates and variances for the upazilas which have only one cluster of observations within them.
...
We now look at the direct estimates and sampling variances if there is only one cluster in an upazila. We use a design effect to do this. The design effect of a single-stage cluster sample, assuming equal selection probabilities and clusters of size M, is given by design effect(henceforth deff or de) = 1 + (M - 1)\* ICC where ICC is the intra-cluster correlation coefficient for the variable of interest. ICC is estimated by the variance of the poorest variable in the cluster divided by the variance of the poorest variable of the cluster added to the variance of the household.

The DHS design involves a second stage of selection of a group of 30 households out of those in the cluster (113 on average) therefore we have set M = 4.

We fit the model using the lme function from the package nlme. STRS stands for stratified simple random sampling.
...
Pre-smoothing this is what the CV looks like:
```{r}
#  2. Test our data  #
...
From the prototype: To reduce this volatility we use an additional smoothing step that involves the use the Generalised Variance Functions. Smoothing the variance estimates may be needed in order to improve the performance of area-level models. Smoothing involves the initial variance estimates being replaced by corresponding predicted values produced under a linear regression model with the response defined by standard error estimates. Predictors in this model include the direct estimates of Poorest, the square of the direct estimates, the number of clusters representing each upazila, the square of the number of clusters and the average sampling weight for a household in each upazila. In order to avoid undue impact of influential observations, the model is fitted by using only the data of upazilas with less than 5 clusters. Such a model explains 62% of the variability present in the data. After fitting the model, only the variances for upazilas represented by 2-4 clusters are smoothed. 

I think this is what we need to do:

1. Calculate the average HH weight for each upazila
2. Fit a linear model where the outcome is standard errors estimates and the predictors are: direct estimates of Poorest, the square of the direct estimates, the number of clusters representing each upazila, the square of the number of clusters and the average sampling weight for a household in each upazila. Use a subset of the upazilas that have fewer than 5 clusters --> what should this model be though??
...
4. Replace the original cve by fitted values for upazilas that have 2-4 clusters.

Note: According to the graph from the prototype I think they also smooth the variance for upazilas that have just one cluster
...
# Test - remove observations where standard errors are 0 (this does not tell us anything about the variability of the estimate)
...
# Keep only observations that have 2-4 clusters - actually I think we should smooth all of them (including upazilas with one cluster)
#fitted <- subset(fitted, num_clusters > 1)
...
NB that we will be inserting Ida's step 2 in here. Now using the final_de dataset for se.GVF
Also the BGD.zonal stats will be used for the geospatial variables
...
NEW CODE with the new method of generating geospatial statistics.
...
We use the column bind command to merge two dataframes together. This new dataframe now includes the variable for "vardir" which is standard errors squared from the generalised variance function (GVF). This is an important argument in the FH model prediction.

### Step 4: Covariate selection for initial model.

This step follows the principle of parsimony in order to slim down the model to find only the geospatial covariates that have the highest predictive power. To do this we first create a new dataframe without the variables that were used to create the wealth index such as whether a household has a mobile phone or their access to sanitation facilities. Since these will clearly be highly correlated with our 'poorest' indicator of interest and we do not want these to be in the model. Also they are obviously not available for the out of sample data points.
...
We trim the dataset of variables since we only want the geospatial variables at this stage and the variable of interest. We have no interest in the standard errors or the upazila code being used for prediction.

We then fit a full linear regression model using the new dataframe


We then perform stepwise selection based on BIC criterion to narrow down the number of variables from 150+ to around 15-20
...
Our first step is running the estimation procedure without carrying out a transformation on the dataset. For this we use the variables that were selected to be most significant by the stepwise selection procedure above.
...
We can then run a couple of model diagnostic regressions to see how the raw scale fits the data.


RAW SCALE ESTIMATION WITH NEW CODE
...
We notice that the model-based point estimates include negative predicted values (min = -2.19) despite the fact that we are modelling a proportion. This is a consequence of using a model that does not fit well and illustrates the importance of using model diagnostics.

As a result in order to better fit the data we are going to try using the arcsin transformation. This is common when using income data. Using this transformation requires additional arguments, namely the effective sample size which is defined by the sample size divided by the design effect, as well as the type of backtransformation.

First of all, in order to do this, we need to generate the argument relating to the effective sample size. First you find out how many households have been surveyed in each upazila. You then load the design effect that we calculated in step 2.

You then merge this with the new_df we are using to make sure we have this information in that survey.
...
summary(fh_arcsin)
```
NB We have initially have set MSE = FALSE above to in order to speed the model stepwise selection step up (the next step) but we will be changing MSE back to true in step 6 when we compute the out of sample estimates.


HERE WE SHOULD PROBABLY CHECK AGAIN THAT MODEL DIAGNOSTICS ARE SATISFIED with the graphs etc
...
Now we are satisfied that this transformation satisfies the model assumptions we can carry on with the remaining modelling steps.
