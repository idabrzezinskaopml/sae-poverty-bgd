...
**Nearest neighbour interpolation - tbc**
:::

## 1. Introduction

A brief description of what I will do in Section A.
...
There are many algorithms that can be used for changing the spatial resolution of a raster. For accessibility and nightlights data, we will use the nearest neighbour interpolation. However, for harmonising demographic data that is of a higher spatial resolution than our base layer, the use of the "sum" algorithm is more appropriate. Otherwise we "lose people" in a downsample. To illustrate the issue, let's imagine we have 9 high resolution pixels (3x3). The sum algorithm will add up all the values from those pixels when aggregating, while the nearest neighbour interpolation
:::
...
Now we are equipped with the tools to start the processing of our geo-spatial datasets. We start by specifying the directory we will be working from and loading all of the data sets described above into our environment and saving them as raster objects.

The code below will automatically pull all the files from each folder based on their \*tif extension. The user should select their corresponding working directory, based on where the geo-spatial data was saved.
