...
@PJ *- Ida - should this not be a specific introduction to this section of the guide? I think that's the general comment that we need an overall structure within which each of the sections fall. Each section can have it's own intro.*
...
@PJ *- Ida, some of these explanations use themselves technical terms that are not straightforward to understand. So for example: "raster data refers to any gridded data..." I am unsure that explains very much? I think we should try - where possible - to explain things in simpler terms. For example: "raster data is data where each observation (row in a table) refers to a specific geographic location or area. The raster is the pattern of locations that together - as a grid - cover a geographic area". I hope that makes sense. Check that the other explanations are similarly clear.*
...
**Vector data** represents structures on the Earth's surface that are defined by discrete locations (x,y) called vertices. Depending on how the vertices (x,y) are organised, we can divide vector data types into: **points, lines, and polygons**.
...
**Nearest neighbour interpolation -** a method for resampling, i.e. changing the spatial resolution of rasters used in remote sensing, where each "corrected" pixel is assigned a data value of the "uncorrected" pixel that is closest to it in terms of coordinate location. @PJ: *Sorry this one is very difficult to get.*

**Resampling using the "sum" algorithm** - a method for resampling, i.e. changing the spatial resolution of a raster used in remote sensing. This is considered the appropriate method when aggregating high-resolution *demographic* data to a coarser grid. @PJ: *As above. Not sure if we want to include the two last ones? They are almost methods that need to be explained in the main text.*
:::
...
@pj: *Ida - I am not sure if all of this is correct. Not all geospatial data is necessarily open source, right? Also not all of it frequently updated. Survey data with geolocation could also be geo-spatial data. See your explanation in section 2.1. So I think we need to change some terminology here?*

The main goal of Section A will be to produce a "**harmonised geo-spatial covariate raster for poverty modelling**". In other words, layers of geo-spatial information in a raster format (a collection of pixels) for each upazila (\@PJ: *Need to explain what an upazila is.*) that we can use to predict poverty. Imagine that each upazila is divided into pixels. Each pixel will contain some geo-spatial information: slope, travel time to healthcare facilities, distance to nearest water source etc. Harmonising them means that all of our geo-spatial layers are in the same format: CRS, extent, spatial resolution, and shape. Why is this necessary? Short answer: so that each pixel with values of geo-spatial data describes the same location across all our geo-spatial datasets.
...
**Raster data** refers to any gridded (or pixelated) data where each pixel represents a specific geographical location. @PJ - *see comments above on this explanation.* The value can be continuous (e.g. temperature) or categorical (e.g. land use). If pixels sound familiar, it's because they are - in fact, this is how every digital image is represented. The only difference is that a raster additionally includes spatial information that connects data from the image to a particular location. The location is defined by the following attributes (these are important attributes to inspect when you first open your raster):
...
Vector data comes in many formats, but we will be working with the (very common) Shapefile format, which has a .shp extension. More specifically, we will download a shapefile containing multipolygons @PJ - *Ida you explained polygons but not multipolygons. Worth explaining them as well.* that define administrative boundaries of upazilas (admin level 3) in Bangladesh. A shapefile stores the coordinates of vertices, as well as other metadata:
...
Could have a short quiz here showing different images and asking the audience whether this is a raster or a vector dataset. @PJ - *yes, good idea.*
...
The next step in the process is to resample the raster, i.e. change the spatial resolution to \~100m to match that of our base layer. Going from a lower to a higher spatial resolution is called disaggregation. If we zoomed into the image under "resample", we would see more smaller pixels appear than previously. Finally, we want to view the "shape" of Bangladesh, rather than a rectangular box bound by its extent. For this purpose, we "mask" our raster layer with nightlights to the base layer - and we finally achieve a nicely formatted raster that resembles Bangladesh!

@PJ: *Ida, you mention this, but it is very difficult to see what is going on in the right three rectangles. To my eye they basically are dark blue, almost uniformly. It would be good if we could do something with this so that it is easier to see variation.*

![](images/Nightlights%20data%20processing.png){width="803"}
...
@PJ: *Ida as before it is very hard to see what is going on here, can we do something about that? The pixels I think are very small.*
...
1.  The `sp` and `sf` packages for vector data @PJ *- Ida, add hyperlinks to these packages.*
...
`sf` objects are organised into three features (see Figure XX below): @PJ: *Ida the below output lists "100 features" in the first row, so this is slightly confusing.*
...
@PJ *- Ida I find this explanation slightly confusing. It is not clear to me at least what these three features in the end are. Why do they matter?*
...
Just a heads up that the files we will be working with are very large. The .zip file itself is is 5GB, while all the files after unzipping will take up around 34GB of space. Please make sure you have sufficient memory on your machine. If you are using an online storage solution, you might want to temporarily offload other files you are not currently using to the cloud. @PJ *- Ida - you should specify how much memory at minimum is required for this.*
:::
...
6.  **World Pop** Pre-harmonised geo-spatial datasets: available for all countries. The datasets includes information on topography, slope, Open Street Map distance to closest road, waterway, and various land cover classes. @PJ *- Ida, explain what this means.* Note that this data has already been harmonised to our base layer and hence will not require any processing in this script.
...
There are many algorithms that can be used for changing the spatial resolution of a raster. For accessibility and nightlights data, we will use the nearest neighbour interpolation, where each "corrected" pixel is assigned a data value of the "uncorrected" pixel that is closest to it in terms of coordinate location. @PJ- *Ida it is unclear what 'corrected' vs 'uncorrected' refers to here. Also unclear why the difference is chosen.* However, for harmonising demographic data that is of a higher spatial resolution than our base layer, the use of the "sum" algorithm is more appropriate. Otherwise we "lose people" in a downsample. To illustrate the issue, let's imagine we have 9 high resolution pixels (3x3). The sum algorithm will add up values from all those pixels when aggregating, while the nearest neighbour interpolation will select the value that falls closest to the "corrected" pixel, and not count any other ones.
:::
...
Now, we have all the raster images with geo-spatial data that we will use in poverty estimation: `matlas.processed`, `fblist.processed`, `nightlights.raster.processed`, and `worldpop.rasters`. These rasters are essentially collections of pixels within Banglafdesh and each pixel has some information associated with it (for example, population density of women). However, now we would like to know which upazila each of these pixels belongs to, so that we can obtain zonal statistics (min, max, mean, sd, etc.). For example: what is the mean nightlights intensity for each upazila?

Before we can do that - we need a file that will allocate each pixel in the raster data to the appropriate upazila. That is, we need to use the shapefile which contains administrative boundaries of upazilas and convert it to a raster of the same spatial resolution, extent, and CRS as the base layer. This newly created raster will assign the correct administrative upazila code to each pixel - and so, will allow us to assign geo-spatial information to the upazila.

First, we want to keep admin codes as numerics. We will create a new column in the shapefile data that will subtract "BD" from the upazila code. So, instead of "BD552769", we will keep simply "552769". This is now saved in the `adm3_integer` column. Next, we will create an empty raster object from scratch and assign the extent, dimensions, and CRS of the base layer. Then, using the `rasterize` function, we will transform values of the admin 3 codes from the shapefile into each pixel of the raster object. A simple plot of the resulting `raster_admin3` shows that values have been assigned to each pixel.
...
This brings us to our final step - calculating zonal statistics, i.e. the values of min, max, mean, sd etc. of all the geo-spatial datasets at upazila level, which will serve as inputs into poverty estimation in Section B. For this, we will use our harmonised rasters from steps 2-5: `matlas.processed`, `fblist.processed`, `nightlights.raster.processed`, and `worldpop.rasters`, as well as a raster that assigns upazila codes to each pixel, raster_admin3, which we obtained from our shapefile in step 6.
...
Now we can calculate our zonal statistics: mean, minimum value, maximum value, sum, standard deviation, and the number of observations. Supplying our raster.stack with all geo-spatial data and raster_admin3, which contains upazila boundaries in the zonal function will calculate all the statistics of interest. We will save those in a data frame format (which should be fairly familiar!), with column names indicating the zonal statistic for each geo-spatial layer. We will combine all statistics in a single data frame zonal.stats.all.
...
Test: using Chris Lloyd's method and the `exactextractr` package

Omg this works way quicker and there is no need to convert the shapefile into a raster. However, some values of min are negative?
