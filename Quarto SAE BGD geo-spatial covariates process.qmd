...
Let us think about the two most common spatial data types, which we will be working with in our poverty mapping exercise, together with their attributes:
...
Make sure that the CRS is the same for all of your objects before layering.
:::

Now, let's see some an example of visualising multiple raster layers from our project work under [Data & Evidence to End Extreme Poverty (DEEP)](https://povertyevidence.org/). In this case, we were interested to map the spatial distribution of climatic risks in Nigeria, looking at droughts and floods. Figure XX below shows the worst drought (defined as the Standardized Precipitation Evapotranspiration Index aka SPEI) and all flooded areas in the period 2010-2020 in Nigeria.
...
-   Operations to be performed on the raster data: crop, mask, resample, replace NA values.

-   How (x,y) values are represented: longitude and latitude, positive and negative values
