...
First of all you have to generate the unique domain identifier for each observation so that you are able to identify each observation by both its upazila as well as its cluster. **Insert Ida's method here:**
...
We now create a variable to determine how many clusters are in each upazila because depending on this, the estimates will be more or less precise due to the change in sample size. The number of clusters in each upazila is necessary so that we know which method we will use to calculate our direct estimates in the next step.
...
load("C:/Users/nickl/iCloudDrive/Desktop/Oxford Policy Management/Bang_V2/BGD.zonalstats.RData")

View(zonal.stats.all)

final_de$ADM3_PCODE <- substring(final_de$ADM3_PCODE, 3)

colnames(final_de)[colnames(final_de) == "ADM3_PCODE"] <- "upazila.code"

new_df <- merge(zonal.stats.all, final_de[, c("upazila.code", "binary_wealth", "se.GVF", "num_clusters")], by = "upazila.code", all.x = TRUE)
...
Generating the nhog variable. first you find out how many households have been surveyed in each upazila.
...
OUR CODE 
```{r}
new_fh_final <- fh(fixed = binary_wealth ~ X201501_Global_Travel_Time_to_Cities_BGD_count +              
X202001_Global_Motorized_Travel_Time_to_Healthcare_BGD_count +
bgd_women_2020_max                                        + 
bgd_women_of_reproductive_age_15_49_2020_max               +  
esaccilc_dst_water_100m_2000_2012_BGD_max                 + 
esaccilc_dst140_100m_2014_BGD_max                          +  
X201501_Global_Travel_Time_to_Cities_BGD_mean             + 
dst_coastline_100m_2000_2020_BGD_mean                     +  
esaccilc_dst_water_100m_2000_2012_BGD_mean                +  
osm_dst_road_100m_2016_BGD_mean                           +  
osm_dst_waterway_100m_2016_BGD_min                         +  
bgd_women_2020_sd                                           + 
bgd_women_of_reproductive_age_15_49_2020_sd                  +
esaccilc_dst130_100m_2014_BGD_sd                            + 
esaccilc_dst140_100m_2014_BGD_sd                            +
X201501_Global_Travel_Time_to_Cities_BGD_sum                 +
bgd_children_under_five_2020_sum   +
bgd_general_2020_sum                +                         
bgd_men_2020_sum                     +                        
bgd_women_of_reproductive_age_15_49_2020_sum  +               
bgd_youth_15_24_2020_sum                       +              
esaccilc_dst140_100m_2014_BGD_sum               +           
osm_dst_road_100m_2016_BGD_sum                   +            
Nightlights_data_sum, vardir = "new_df$se.GVF^2",
...
summary(new_fh_final)
```
OUR CODE using second bit of data 

```{r}
new_fh_final <- fh(fixed = binary_wealth ~ min.osm_dst_waterway_100m_2016_BGD + 
...
OUR NEW CODE 
```{r}
fh_arcsin <- fh(fixed = binary_wealth ~ X201501_Global_Travel_Time_to_Cities_BGD_count +              
X202001_Global_Motorized_Travel_Time_to_Healthcare_BGD_count +
bgd_women_2020_max                                        + 
bgd_women_of_reproductive_age_15_49_2020_max               +  
esaccilc_dst_water_100m_2000_2012_BGD_max                 + 
...
```{r}
fh_arcsin_final <- fh(fixed = binary_wealth ~ X201501_Global_Travel_Time_to_Cities_BGD_count + 
    X202001_Global_Motorized_Travel_Time_to_Healthcare_BGD_count + 
    bgd_women_2020_max + bgd_women_of_reproductive_age_15_49_2020_max + 
    X201501_Global_Travel_Time_to_Cities_BGD_mean + dst_coastline_100m_2000_2020_BGD_mean + 
    osm_dst_road_100m_2016_BGD_mean + osm_dst_waterway_100m_2016_BGD_min + 
    esaccilc_dst130_100m_2014_BGD_sd + X201501_Global_Travel_Time_to_Cities_BGD_sum + 
    bgd_children_under_five_2020_sum + bgd_general_2020_sum + 
    bgd_men_2020_sum + bgd_women_of_reproductive_age_15_49_2020_sum + 
    bgd_youth_15_24_2020_sum + osm_dst_road_100m_2016_BGD_sum + 
    Nightlights_data_sum, vardir = "new_df$se.GVF^2", combined_data = cleaned_df, 
    domains = "upazila.code", method = "ml", transformation = "arcsin", 
    backtransformation = "bc", eff_smpsize = "num_unique_hhid", 
    MSE = TRUE, mse_type = "boot", B = 50)
